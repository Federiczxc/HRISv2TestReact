...
export default function ot_reports_list({ OTReportsList }) {
    const [file, setFile] = useState(null);
    const [opened, setOpened] = useState(false);
    const [previewFile, setPreviewFile] = useState([]);
    const open = () => setOpened(true);
    const close = () => setOpened(false);
    const data = OTReportsList
    ...
    const handleExportTemplate = () => {
        ...
        XLSX.writeFile(workbook, 'OT_Reports_List_Template.xls');
 ...
 */
    ...
    const handleUpload = () => {
        if (file) {
            if (!file.name.toLowerCase().includes("ot")) {
                notifications.show({
                    title: 'Error',
                    message: 'It must be an OT File!',
                    color: 'red',
                    position: 'top-right',
                    autoClose: 5000,
                });
            ...
            reader.onload = (event) => {
                ...
                const hasOT = columns.some(col => col.toLowerCase().includes("ot"));
    ...
    function handleUploadData(parsedData) {
        ...
        const updatedValue = { ot_upload: updatedData };
        axios.post('/OT_Module/ot_reports_list', updatedValue) //inertia wont work
            .then((response) => {
                notifications.show({
                    title: 'Success',
                ...
                setFile(null);
                setPreviewFile([]);
                router.visit('/OT_Module/ot_reports_list', { //USE THIS SHIT IF AXIOS GAMIT no need usestate
                    only: ['OTReportsList'],
                })
                close();
            })
